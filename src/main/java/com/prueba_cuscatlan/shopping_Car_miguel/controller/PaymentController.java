...
public class PaymentController {
    ...
    @Operation(summary = "Submit a payment (async)", description = """
            ...
            """)
    @ApiResponses({
            @ApiResponse(responseCode = "202", description = "Payment accepted — processing asynchronously", headers = @Header(name = "Location", description = "URL to poll for payment status", schema = @Schema(type = "string", example = "/api/v1/payments/42")), content = @Content(schema = @Schema(implementation = OrderPaymentResponse.class))),
            @ApiResponse(responseCode = "400", description = "Amount mismatch, order already paid, or validation error", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
    ...
    ResponseEntity<OrderPaymentResponse> processPayment(
        ...
        OrderPaymentResponse response = paymentService.processPayment(principal.getName(), idempotencyKey, request);
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Payment found", content = @Content(schema = @Schema(implementation = OrderPaymentResponse.class))),
    ...
    @GetMapping("/{id}")
    ResponseEntity<OrderPaymentResponse> findById(Principal principal, @PathVariable Long id) {
        return ResponseEntity.ok(paymentService.findById(principal.getName(), id));
    }
}
